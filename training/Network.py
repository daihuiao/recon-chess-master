...
class ReconChessNet(Model):
	...
	def get_lstm(self, x, inference=True,mask=None):
		...
		batch_size=len(x)
		x = pad_sequences(x,padding='post')#填充序列使文本集中所有文本长度相同。
		...
		x = tf.cast(x,tf.float32)  # 函数 tf.cast() cast( x, dtype, name=None ) 将x的数据格式转化成dtype.
		x = self.conv1(x)
		x = self.conv2(x)
		x = self.conv3(x)
		x = self.conv4(x)
	...
	def loss(self,inputs,mask,lg_prob_old,a_taken,GAE,old_v_pred,returns,clip):
		...
		lg_prob_old = tf.reshape(pad_sequences(lg_prob_old,padding='post',dtype=np.float32),(-1,))#todo 这里用0填充合适吗
	...
	def update(self,inputs,mask,lg_prob_old,a_taken,GAE,old_v_pred,returns,clip):
		...
		for i in range(n_iters):
			...
			#EAGER EXECUTION BUG MEMORY LEAK          eager execution
