...
class ReconTrainer:
    ...
    def play_game(self,train_as_white,maximum_moves):
        ...
        game = LocalGame(seconds_per_player=999999)
    ...
    def collect_exp(self,n_moves,max_turns_per_game,loop):
        ...
        while moves_played<n_moves:
            ...
            if moves_played + max_turns_per_game*2 > n_moves:
                max_turns_per_game = (n_moves - moves_played)//2#保证得到的经验数刚好是设置的值
            train_as_white = random.choice([True,False])
            outmem,moves,game_history,op_num = self.play_game(train_as_white,max_turns_per_game)#memory = [obs_memory,mask_memory,action_memory,rewards,gae]
        ...
        cpu_comm.barrier()#todo  MPI_BARRIER阻塞所有的调用者直到所有的组成员都调用了它，各个进程中这个调用才可以返回。就是说，有些进程执行得快，有些进程执行得慢，要等待所有的进程都执行到这里，才开时同时执行之后的命令，即“同步”。
        if rank==1:
            ...
            wandb.log({'rank1_performance/loop': loop,
                       ...
                       ' rank1_performance/ties':tot_ties/ngames,
                       ' rank1_performance/mean_score':np.mean(self.score),
        ...
        while True:
            if rank != 0:
                ...
                if np.amin(np.mean(self.score,0)) >= equalize_weights_on_score:
                    ...
                    wandb.log({"equalize/min_score":np.amin(np.mean(self.score,0)),
                               "equalize/equalize_weights_on_score":equalize_weights_on_score,
                               "equalize/self.next_to_sync_with_train":self.next_to_sync_with_train,
                               })
